...
public class ExamService {
    ...
    public Exam createExam(Exam exam) {
        ...
        if (exam.getTitle() == null || exam.getTitle().isBlank()) throw new IllegalArgumentException("Exam title is required");
        if (exam.getExamDate() == null) throw new IllegalArgumentException("Exam date is required");
        if (!exam.getExamDate().isAfter(LocalDate.now())) throw new IllegalArgumentException("Exam date must be in the future");
        if (exam.getTotalMarks() == null || exam.getTotalMarks() <= 0) throw new IllegalArgumentException("Total marks must be greater than zero");
        if (exam.getCourse() == null || exam.getCourse().getId() <= 0) throw new IllegalArgumentException("Course ID is required");
    ...
    public Optional<Exam> updateExam(Long id, Exam exam) {
        if (id == null || id <= 0 || exam == null) return Optional.empty();
        Optional<Exam> existingExam = examRepository.findByIdAndIsActiveTrue(id);
        if (existingExam.isEmpty()) return Optional.empty();
    ...
    public boolean softDeleteExam(Long id) {
        ...
        Optional<Exam> existingExam = examRepository.findByIdAndIsActiveTrue(id);
        if (existingExam.isEmpty()) return false;
